...
def mlsealselect (timegroupcases, class_object, shape_object):
	...
	subset = timegroupcases.filter(face__fk_shape=shape_object).filter(face__fk_class=class_object)[:10]
...
def mltrainmodel():
	## function handles the creation of the ML model and then updates all cases used in training
...
def mltrainset():
	...
	face_objectset = Face.objects.filter(
